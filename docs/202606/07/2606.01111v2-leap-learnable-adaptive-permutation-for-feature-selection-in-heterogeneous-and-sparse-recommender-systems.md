...
title_zh: LeAP：面向异构稀疏推荐系统的可学习自适应特征选择排列
...
score: 7.0
evidence: 推荐系统的特征选择
tldr: 针对现代推荐系统中特征维度异构、极端稀疏且排列评估计算昂贵的问题，提出可学习自适应排列(LeAP)模块，将随机排列转化为可学习机制并引入自适应正则化，高效评估特征重要性。在公开数据集上达到最优，并在工业级模型（超万亿参数、十亿日请求）中成功移除3600+冗余维度，性能无损。
source: arxiv
...
motivation: 现有特征选择方法假设特征维度统一、难以处理极稀疏特征，且排列过程计算成本过高。
method: 提出LeAP，一个可学习自适应排列插件，将随机排列转化为可学习机制，并引入针对异构维度和极端稀疏的自适应正则化策略。
result: 在四个公开推荐数据集上达到最优，在超万亿参数、日请求十亿的工业搜索排序模型中成功删除3600+冗余维度，无性能损失，能力为基线方法的2-10倍。
conclusion: LeAP是一种高效、可部署于大规模分布式系统的异构稀疏特征选择方法，显著降低计算成本并保持预测精度。
---

## 摘要
现代工业推荐系统依赖数千种异构特征——从低维标量（如统计值）到高维嵌入（如用户ID嵌入、MLP表示）——以实现高精度预测。由于训练相关的巨大计算成本，高效的特征选择至关重要。然而，现有方法面临三个主要瓶颈：（1）通常假设特征维度统一或需要昂贵的映射到固定大小；（2）难以处理极端稀疏性，其中大多数特征（例如99%以上）保持默认值；（3）传统的基于排列的方法在大规模设置中计算成本过高。为应对这些挑战，我们提出LeAP（可学习自适应排列），一种新颖的、模型无关的特征选择插件模块。LeAP将低效的随机排列过程转化为可学习机制，显著加速特征重要性评估。此外，我们引入了一种针对异构维度和极端稀疏性定制的自适应正则化策略，能够在非对称输入空间上实现优越的特征重要性排序结果。在四个公开推荐数据集上的实验表明，LeAP达到了最先进的性能。此外，LeAP已部署在一个每日请求超过十亿、模型参数规模达2TB的大规模工业搜索排序模型中。在这个包含12000+总特征维度的实际场景中，LeAP成功识别并移除了超过3600个冗余维度，且性能未下降，这比对比基线方法的能力高出2到10倍。
