...
tags: ["query:q7"]
score: 8.0
evidence: 大模型需求模拟用于定价
tldr: "本文提出一种基于大语言模型的虚拟人口模拟方法，通过有限混合客户角色和LLM生成购买概率，聚合得到需求分布，用于定价决策。该方法利用产品文本和图像等非结构化信息，支持期望收益和风险意识目标。在H&M数据集上，该模型预测性能最优，能高效进行反事实定价，特别适用于历史数据有限但产品信息丰富的场景。"
source: arxiv
...
motivation: 定价决策中，产品信息丰富但历史需求有限，需要既能预测均值又能量化不确定性的需求模拟方法。
method: 构建基于LLM的虚拟人口模型，用有限混合客户角色表示顾客，对每个角色、产品和价格，LLM融合结构化与非结构化信息输出购买概率，经校准权重聚合为需求预测分布。
result: "在H&M数据集上，校准后的LLM模拟器在所有对比模型中预测性能最佳，并支持样本有效的风险感知定价。"
conclusion: 所提框架提供了一种实用方式，将LLM作为需求模拟器，通过输出完整需求分布而非点预测，支持管理者进行价格比较、不确定性量化和目标导向定价。
---

## 摘要
我们开发了一个基于LLM的虚拟人口模型，用于模拟定价决策的需求，其中产品由丰富的非结构化信息（如文本描述和图像）描述，决策者不仅需要平均需求预测，还需要反事实价格的不确定性估计。我们的模型将暴露的客户表示为从有限混合的客户角色中抽取的样本。对于每个角色、产品和候选价格，LLM使用结构化角色信息和非结构化产品信息来引出角色级别的购买概率。这些概率通过校准的混合权重聚合，形成总需求的预测分布。生成的模拟器可以在各种定价目标下评估反事实价格，包括期望收入和风险意识标准（如条件风险价值）。我们在一个包含产品描述和图像的在线H&M时尚数据集上测试了该框架。校准后的基于LLM的模拟器在所考虑的模型中实现了最佳的整体预测性能，并支持样本高效的定价决策。我们的框架提供了一种实用的方法，将LLM用作需求模拟器，适用于历史需求数据有限但产品信息丰富的产品。通过生成完整的预测需求分布而不仅仅是点预测，它使管理者能够比较候选价格、量化需求不确定性，并选择针对平均收入或风险意识目标的定价。
